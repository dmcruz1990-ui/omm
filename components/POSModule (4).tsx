...
const ServiceOSModule: React.FC<POSProps> = ({ tables, onUpdateTable, onOpenVisionAI }) => {
  ...
  const abrirEncuesta = (tableId: number) => {
    ...
    setModal({
      open: true, title: '',
      content: (
        <div className="font-['DM_Sans'] text-center">
          <div className="text-[40px] mb-3">⭐</div>
          <div className="font-['Syne'] text-[20px] font-bold mb-1">¿Cómo fue tu experiencia?</div>
          <div className="text-[13px] text-[#a0a0a0] mb-6">{m.cliente} · OMM · Mesa {m.num}</div>
          ...
          <div className="flex justify-center gap-3 mb-6">
            {[1, 2, 3, 4, 5].map(star => (
              <button key={star} onClick={() => {
                closeModal();
                showToast(`⭐ ¡Gracias! ${star} estrella${star > 1 ? 's' : ''} — Hasta pronto 🙏`);
                setOrder(prev => prev.filter(o => o.mesa !== m.num));
              }}
                className="w-14 h-14 rounded-2xl bg-[#222] border border-[#2a2a2a] text-[28px] hover:bg-[#d4943a]/10 hover:border-[#d4943a] transition-all flex items-center justify-center">
                {star <= 3 ? ['😞', '😐', '🙂', '😊', '🤩'][star - 1] : ['😞', '😐', '🙂', '😊', '🤩'][star - 1]}
              </button>
            ))}
          </div>

          <div className="text-[11px] text-[#606060] mb-4">Toca una opción para registrar tu opinión</div>

          {/* Premio */}
          <div className="bg-[#d4943a]/10 border border-[#d4943a]/30 rounded-xl p-3 mb-4 flex items-center gap-3">
            <span className="text-[28px]">🎁</span>
            <div className="text-left">
              <div className="text-[13px] font-bold text-[#d4943a]">¡Gana un premio!</div>
              <div className="text-[11px] text-[#a0a0a0]">Responde y obtén 10% off en tu próxima visita</div>
            </div>
          </div>

          <button onClick={() => closeModal()}
            className="text-[11px] text-[#606060] hover:text-[#a0a0a0] underline">
            Omitir encuesta
          </button>
        </div>
      ),
