...
const ServiceOSModule: React.FC<POSProps> = ({ tables, onUpdateTable, onOpenVisionAI }) => {
  ...
  // MARCHAR AHORA — va directo a cocina, descuenta stock, avisa si hay cola larga
  const marcharAhora = (p: any) => {
    ...
    // Contar pedidos en cocina — avisar demora si hay más de 10
    const totalEnCocina = order.length + pendingOrder.length;
    if (totalEnCocina >= 10) {
      showToast(`⏱ ${p.nombre} marchando → Cocina cargada (+10 pedidos) — posible demora`);
    } else {
          ...
          <div className="grid grid-cols-2 gap-1.5 mb-2">
            {[
              ...
              <div key={mp.label} className="border border-[#2a2a2a] rounded-xl p-2.5 cursor-pointer hover:border-[#606060] transition-all">
                ...
                {mp.sub && <div className="text-[9px] text-[#606060] mb-1.5">{mp.sub}</div>}
            ...
            {/* Bono + Tarjeta Regalo — botón especial */}
            <div className="border border-[#9b72ff]/40 bg-[#9b72ff]/05 rounded-xl p-2.5">
              <div className="flex items-center gap-2 mb-1.5">
                <span className="text-[16px]">🎁</span>
                <span className="text-[11px] font-semibold text-[#9b72ff]">Bono / Regalo</span>
              </div>
              <button onClick={() => abrirBonoRegalo(tableId, total)}
                style={{ background: '#9b72ff18', borderColor: '#9b72ff50', color: '#9b72ff' }}
                className="w-full py-1.5 rounded-lg border text-[9px] font-bold transition-all hover:opacity-90 active:opacity-70">
                🎟 Canjear código
              </button>
            </div>
          </div>
