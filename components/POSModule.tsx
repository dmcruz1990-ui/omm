...
const ServiceOSModule: React.FC<POSProps> = ({ tables, onUpdateTable, onOpenVisionAI }) => {
  ...
  const abrirPOS = (tableId: number) => {
    ...
    const propina = Math.round(subtotalNeto * 0.10);
    ...
    setModal({
      ...
      content: (
        <div className="font-['DM_Sans']">
          ...
          {/* Items */}
          <div className="bg-[#0a0a0a] rounded-[10px] overflow-hidden mb-3 max-h-[200px] overflow-y-auto">
            ...
            {items.map((o, i) => (
              <div key={i} className="px-3 py-2 border-b border-[#2a2a2a] grid grid-cols-[1fr,auto,auto] gap-2 items-center">
                ...
                <button onClick={() => removeOrder(order.findIndex(x => x === o))} className="text-[#e05050] text-[12px]">✕</button>
              </div>
            ))}
          </div>
          {/* Descuentos */}
          <div className="bg-[#0a0a0a] rounded-[10px] p-3 mb-3">
            <div className="text-[10px] text-[#606060] font-bold uppercase tracking-wider mb-2">Ajustes</div>
              ...
              <div className="flex gap-1">
                {[0, 5, 10, 15, 20].map(p => (
                  <button key={p} onClick={() => { setPosDescuento(p); abrirPOS(tableId); }}
                    className={`px-2.5 py-1 rounded-md text-[11px] font-semibold border transition-all ${posDescuento === p ? 'border-[#d4943a] bg-[#d4943a]/15 text-[#d4943a]' : 'border-[#2a2a2a] text-[#606060]'}`}>
                    {p}%
                  </button>
                ))}
              </div>
              ...
              <div className="flex gap-1">
                {[0, 5000, 10000, 20000].map(v => (
                  <button key={v} onClick={() => { setPosCorte(v); abrirPOS(tableId); }}
                    className={`px-2.5 py-1 rounded-md text-[11px] font-semibold border transition-all ${posCorte === v ? 'border-[#d4943a] bg-[#d4943a]/15 text-[#d4943a]' : 'border-[#2a2a2a] text-[#606060]'}`}>
                    {v === 0 ? '—' : `$${formatPrecio(v)}`}
                  </button>
                ))}
              </div>
          ...
          <div className="bg-[#0a0a0a] rounded-[10px] p-3 mb-3 flex flex-col gap-1.5">
            ...
            <div className="flex justify-between text-[12px] text-[#606060]"><span>Propina sugerida (10%)</span><span>${formatPrecio(propina)}</span></div>
            <div className="flex justify-between text-[15px] font-bold pt-2 border-t border-[#2a2a2a] mt-1">
              <span>Total</span><span className="text-[#f0b45a]">${formatPrecio(total)}</span>
            </div>
          </div>
          {/* Métodos de pago */}
          <div className="flex gap-2 mb-3">
            {['💳 Tarjeta', '💵 Efectivo', '📱 Nequi', '🏦 Transferencia'].map(m => (
              <button key={m} onClick={() => { closeModal(); showToast(`✓ Pago procesado — ${m}`); setOrder(prev => prev.filter(o => o.mesa !== selectedTable.num)); }}
                className="flex-1 py-2 rounded-lg border border-[#2a2a2a] text-[10px] text-[#a0a0a0] hover:border-[#d4943a] hover:text-[#d4943a] transition-all text-center">
                {m}
              </button>
            ))}
          </div>
          <button onClick={() => { closeModal(); showToast(`✓ Mesa ${m.num} cobrada y cerrada`); setOrder(prev => prev.filter(o => o.mesa !== m.num)); }}
            className="w-full py-3 rounded-xl bg-[#d4943a] text-black font-bold text-[14px] hover:bg-[#f0b45a] transition-all">
  ...
  return (
    <div className="flex h-full w-full bg-[#0a0a0a] text-[#f0f0f0] overflow-hidden -m-12" style={{ fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div className="flex-1 p-3.5 overflow-y-auto">
          ...
          <div className="grid gap-2.5" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))' }}>
            {(productos[currentCat] || []).map((p, i) => {
              ...
              return (
                <div key={i} onClick={() => addToOrder(p)}
                  className={`bg-[#1c1c1c] border rounded-xl overflow-hidden cursor-pointer transition-all flex flex-col relative hover:border-[#d4943a]/50 hover:-translate-y-0.5 ${isAdded ? 'border-[#3dba6f]' : 'border-[#2a2a2a]'}`}>
                  <div className="w-full aspect-[4/3] bg-[#222] flex items-center justify-center text-[44px]">{p.emoji}</div>
                  <div className="p-2.5 flex flex-col gap-1 flex-1">
                    <div className="text-[13px] font-bold text-[#f0f0f0] leading-tight">{p.nombre}</div>
                    {desc && <div className="text-[10px] text-[#606060] leading-snug flex-1">{desc}</div>}
                    ...
                    <div className="text-[14px] font-bold text-[#d4943a]">{p.precio}</div>
                    <button onClick={(e) => { e.stopPropagation(); addToOrder(p); }}
                      className="w-full py-1.5 rounded-lg bg-[#222] border border-[#2a2a2a] text-[11px] font-semibold text-[#a0a0a0] hover:bg-[#d4943a] hover:text-black hover:border-[#d4943a] transition-all mt-1">
                      + Agregar
                    </button>
                  </div>
                  {isAdded && (
                    <div className="absolute inset-0 bg-[#3dba6f]/15 flex items-center justify-center text-[36px]">✓</div>
                  )}
                </div>
              );
            })}
