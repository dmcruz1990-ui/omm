...
const ServiceOSModule: React.FC<POSProps> = ({ tables, onUpdateTable, onOpenVisionAI }) => {
  ...
  // MARCHAR AHORA — va directo a cocina, descuenta stock
  const marcharAhora = (p: any) => {
    showToast(`🍽️ ${p.nombre} marchando → Cocina`);
    setAddedCards(prev => new Set([...prev, p.nombre + '_marchar']));
    setStockFlow(prev => ({ ...prev, [p.nombre]: Math.max(0, (prev[p.nombre] ?? 10) - 1) }));
    setTimeout(() => setAddedCards(prev => { const n = new Set(prev); n.delete(p.nombre + '_marchar'); return n; }), 1500);
    setSelectedPlato(null);
  };

  // AGREGAR A LA ORDEN — va al pendingOrder izquierdo para confirmar
      ...
      content: (
        <div className="font-['DM_Sans']">
          ...
          <div className="bg-[#0a0a0a] rounded-[10px] p-3 mb-3">
            <div className="text-[10px] text-[#d4943a] font-bold uppercase tracking-wider mb-2.5 flex items-center gap-1.5">
              🔐 Ajustes Gerente
            </div>

            {/* Descuento % */}
            <div className="mb-2.5">
              <div className="text-[10px] text-[#606060] mb-1.5">Descuento %</div>
              ...
              <div className="flex items-center gap-2">
                <input type="number" min={0} max={100} placeholder="% libre"
                  className="w-20 bg-[#141414] border border-[#2a2a2a] rounded-md px-2 py-1 text-[11px] text-[#f0f0f0] outline-none focus:border-[#d4943a]"
                  onBlur={e => { const v = parseInt(e.target.value); if (!isNaN(v) && v >= 0 && v <= 100) { setPosDescuento(v); abrirPOS(tableId); } }}
                />
                <span className="text-[10px] text-[#606060]">= -{formatPrecio(descuento > 0 ? descuento : 0)}</span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-1.5 mb-3">
            {[
              ...
              <div key={mp.label} className="border border-[#2a2a2a] rounded-xl p-2.5 cursor-pointer hover:border-[#606060] transition-all">
                <div className="flex items-center gap-2 mb-1.5">
                  <span className="text-[16px]">{mp.icon}</span>
                  <span className="text-[11px] font-semibold text-[#f0f0f0]">{mp.label}</span>
                </div>
                <div className="flex flex-col gap-1">
                  <button onClick={() => procesarPago(mp.label, false)}
                    className="w-full py-1 rounded-lg border border-[#2a2a2a] text-[9px] font-semibold text-[#a0a0a0] hover:bg-[#2a2a2a] active:bg-[#3dba6f]/20 transition-all">
                  ...
                  <button onClick={() => procesarPago(mp.label, true)}
                    ...
                    className="w-full py-1 rounded-lg border text-[9px] font-bold transition-all hover:opacity-90 active:opacity-70">
    ...
    <div ref={wrapperRef} className="flex bg-[#0a0a0a] text-[#f0f0f0]" style={{ 
      ...
      <div className="flex-1 flex flex-col bg-[#0a0a0a] min-w-0">
        ...
        <div className="bg-[#141414] border-b border-[#2a2a2a] px-3 flex items-center h-[44px] shrink-0">
          ...
          <div className="ml-auto flex items-center gap-1 shrink-0">
            {(['IA','Cuenta','Chat'] as const).map(tab => {
              ...
              const isActive = drawerOpen && rightTab === tab;
              return (
                <button key={tab} onClick={() => { setRightTab(tab); setDrawerOpen(p => rightTab === tab ? !p : true); }}
                  className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg border text-[11px] font-bold transition-all ${isActive ? 'bg-[#d4943a]/15 border-[#d4943a] text-[#d4943a]' : 'bg-[#1c1c1c] border-[#2a2a2a] text-[#606060] hover:text-[#a0a0a0]'}`}>
                  {icons[tab]}<span className="hidden lg:inline">{tab}</span>
                </button>
              );
            })}
            <button onClick={() => setShowOrderPanel(!showOrderPanel)}
              className={`ml-1 w-9 h-9 rounded-lg border flex items-center justify-center relative transition-all ${order.length > 0 ? 'bg-[#d4943a]/10 border-[#d4943a] text-[#d4943a]' : 'bg-[#1c1c1c] border-[#2a2a2a] text-[#a0a0a0] hover:text-[#d4943a]'}`}>
        ...
        <div className="bg-[#0a0a0a] border-t border-[#2a2a2a] flex flex-col shrink-0">
          ...
          {!barraColapsada && (
            <>
              {/* FILA RITUAL — todas las mesas */}
              <div className="border-b border-[#1a1a1a] overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
                <div className="flex items-center px-1 py-1 min-w-max gap-0">
                  {displayTables.map((m) => {
                    ...
                    return (
                      <div key={m.id} className={`flex items-center gap-0.5 px-1.5 py-0.5 border-r border-[#1a1a1a] last:border-r-0 shrink-0 ${!isActive ? 'opacity-50' : ''}`}>
                        <button onClick={() => setSelectedTableId(m.id)}
                          className={`text-[8px] font-black px-1.5 py-0.5 rounded mr-0.5 transition-all shrink-0 ${isActive ? 'bg-[#d4943a] text-black' : 'bg-[#1a1a1a] text-[#606060] hover:text-[#d4943a]'}`}>
                          M{m.num}
                        </button>
                        {ritualStepsAll.map((step) => {
                          ...
                          return (
                            <button key={step} onClick={() => toggleRitualStep(m.id, step)} title={step}
                              style={done ? { background: activeBg, borderColor: activeColor+'80', color: activeColor } : { background: 'transparent', borderColor: '#1a1a1a', color: '#333' }}
                              className="flex items-center gap-0.5 px-1 py-0.5 rounded border text-[8px] font-bold whitespace-nowrap transition-all shrink-0 hover:opacity-90 active:scale-95">
                              <span style={{ fontSize: 10 }}>{done ? '✓' : stepEmojis[step]}</span>
                              <span style={{ fontSize: 8 }}>{step === 'Robata/Wok' ? 'Rob' : step === 'Pousse-café' ? 'Pouss' : step === 'Recomendar' ? 'Rec' : step.split('/')[0]}</span>
                            </button>
                          );
                        })}
                      </div>
      ...
      {/* RIGHT PANEL — DRAWER FLOTANTE */}
      {drawerOpen && (
        <>
          {/* Overlay semitransparente */}
          <div className="fixed inset-0 z-[300]" style={{ background: 'rgba(0,0,0,0.35)' }}
            onClick={() => setDrawerOpen(false)} />
          {/* Panel flotante */}
          <div className="fixed right-0 top-0 h-full z-[400] bg-[#141414] border-l border-[#2a2a2a] flex flex-col shadow-2xl" style={{ width: 300 }}>
        <div className="p-3 px-4 border-b border-[#2a2a2a] flex items-center gap-2.5 shrink-0">
          ...
          <button onClick={() => setDrawerOpen(false)} className="ml-auto w-8 h-8 rounded-lg bg-[#1c1c1c] border border-[#2a2a2a] flex items-center justify-center text-[#606060] hover:text-white hover:border-[#606060] transition-all">
            <X size={14} />
          </button>
        </div>
