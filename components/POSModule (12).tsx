  ...
  return (
    <div ref={wrapperRef} className="flex bg-[#0a0a0a] text-[#f0f0f0]" style={{ 
          ...
          {(() => {
            ...
            return (
              <>
                ...
                {/* BRIEF — Producto en 86 unidades */}
                <div className="mt-3 pt-3 border-t border-[#2a2a2a]">
                  <div className="text-[10px] font-semibold uppercase tracking-[0.8px] text-[#606060] mb-2 flex items-center gap-1.5">
                    Producto en <span className="text-[#e05050] font-black">86</span> unidades
                  </div>
                  <div className="flex flex-col gap-1">
                    {[{ icon: '🦑', name: 'Calamares Fritos' }, { icon: '🧀', name: 'Tarta de Queso' }].map(item => (
                      <div key={item.name} className="flex items-center gap-2 py-1.5 border-b border-[#1a1a1a] last:border-0">
                        <span className="text-[14px]">{item.icon}</span>
                        <span className="flex-1 text-[11px] text-[#a0a0a0]">{item.name}</span>
                        <button onClick={() => showToast(`📋 ${item.name} reportado a stock`)}
                          className="w-[22px] h-[22px] rounded-md bg-[#1a1a1a] border border-[#2a2a2a] flex items-center justify-center hover:bg-[#d4943a] hover:text-black hover:border-[#d4943a] active:bg-[#3dba6f] active:border-[#3dba6f] transition-all text-[12px]">+</button>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <div className="flex-1 flex flex-col bg-[#0a0a0a] min-w-0">
        ...
        {/* BARRA INFERIOR — Ritual completo por mesa en una sola línea */}
        <div className="bg-[#0f0f0f] border-t border-[#2a2a2a] flex flex-col shrink-0">

          {/* Fila de rituales por mesa — una línea horizontal completa */}
          <div className="border-b border-[#2a2a2a] overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
            <div className="flex items-center gap-0 px-2 py-1.5 min-w-max">
              {displayTables.map((m, mi) => {
                const state = ritualState[m.id] || [];
                const isActive = m.id === selectedTableId;
                return (
                  <div key={m.id}
                    className={`flex items-center gap-1 shrink-0 px-2 border-r border-[#2a2a2a] last:border-r-0 ${isActive ? '' : 'opacity-60'}`}>
                    {/* Label mesa */}
                    <button onClick={() => setSelectedTableId(m.id)}
                      className={`text-[9px] font-black tracking-wider shrink-0 mr-1 px-1.5 py-0.5 rounded-md transition-all ${isActive ? 'bg-[#d4943a] text-black' : 'text-[#606060] hover:text-[#a0a0a0]'}`}>
                      M{m.num}
                    </button>
                    {/* Pasos del ritual */}
                    {ritualStepsAll.map((step) => {
                      ...
                      const stepColors: Record<string, string> = {
                        'Agua': done ? 'bg-[#4a8fd4]/20 border-[#4a8fd4]/60 text-[#4a8fd4]' : 'border-[#2a2a2a] text-[#404040] hover:border-[#4a8fd4]/40 hover:text-[#4a8fd4]',
                        'Coctel': done ? 'bg-[#9b72ff]/20 border-[#9b72ff]/60 text-[#9b72ff]' : 'border-[#2a2a2a] text-[#404040] hover:border-[#9b72ff]/40 hover:text-[#9b72ff]',
                        'Compartir': done ? 'bg-[#d4943a]/20 border-[#d4943a]/60 text-[#d4943a]' : 'border-[#2a2a2a] text-[#404040] hover:border-[#d4943a]/40 hover:text-[#d4943a]',
                        'Robata/Wok': done ? 'bg-[#e05050]/20 border-[#e05050]/60 text-[#e05050]' : 'border-[#2a2a2a] text-[#404040] hover:border-[#e05050]/40 hover:text-[#e05050]',
                        'Postre': done ? 'bg-[#f0b45a]/20 border-[#f0b45a]/60 text-[#f0b45a]' : 'border-[#2a2a2a] text-[#404040] hover:border-[#f0b45a]/40 hover:text-[#f0b45a]',
                        'Recomendar': done ? 'bg-[#3dba6f]/20 border-[#3dba6f]/60 text-[#3dba6f]' : 'border-[#2a2a2a] text-[#404040] hover:border-[#3dba6f]/40 hover:text-[#3dba6f]',
                        'Pousse-café': done ? 'bg-[#3dba6f]/20 border-[#3dba6f]/60 text-[#3dba6f]' : 'border-[#2a2a2a] text-[#404040] hover:border-[#3dba6f]/40 hover:text-[#3dba6f]',
                        'Café/Té': done ? 'bg-[#a0522d]/30 border-[#a0522d]/60 text-[#cd853f]' : 'border-[#2a2a2a] text-[#404040] hover:border-[#a0522d]/40 hover:text-[#cd853f]',
                        'Vino': done ? 'bg-[#8b1a4a]/30 border-[#c2185b]/60 text-[#e91e8c]' : 'border-[#2a2a2a] text-[#404040] hover:border-[#c2185b]/40 hover:text-[#e91e8c]',
                        'Licor': done ? 'bg-[#ffd700]/15 border-[#ffd700]/50 text-[#ffd700]' : 'border-[#2a2a2a] text-[#404040] hover:border-[#ffd700]/40 hover:text-[#ffd700]',
                      };
                      const stepEmojis: Record<string, string> = {
                        'Agua':'💧','Coctel':'🍹','Compartir':'🥟','Robata/Wok':'🔥','Postre':'🍮',
                        'Recomendar':'⭐','Pousse-café':'🥃','Café/Té':'☕','Vino':'🍷','Licor':'🥂'
                      };
                      return (
                        <button key={step}
                          onClick={() => toggleRitualStep(m.id, step)}
                          title={`${done ? '✓ ' : ''}${step} — Mesa ${m.num}`}
                          className={`flex items-center gap-0.5 px-1.5 py-0.5 rounded-md border text-[9px] font-semibold cursor-pointer whitespace-nowrap transition-all shrink-0 ${stepColors[step] || (done ? 'bg-[#3dba6f]/15 border-[#3dba6f]/40 text-[#3dba6f]' : 'border-[#2a2a2a] text-[#404040]')}`}>
                          <span className="text-[11px]">{done ? '✓' : stepEmojis[step]}</span>
          ...
          {/* Fila de quick-add productos por categoría */}
          <div className="flex items-stretch gap-0 border-b border-[#1a1a1a] overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
            {[
              { cat: 'Agua', emoji: '💧', color: '#4a8fd4', items: [
                { nombre: 'Con Gas', precio: '$3.000', emoji: '💧' },
                { nombre: 'Sin Gas', precio: '$3.000', emoji: '🫧' },
              ]},
              { cat: 'Coctel', emoji: '🍹', color: '#9b72ff', items: [
                { nombre: 'Yin Peng', precio: '$49.900', emoji: '🍹' },
                { nombre: 'Infinito', precio: '$54.800', emoji: '🍍' },
                { nombre: 'Gin Ken', precio: '$56.400', emoji: '🍸' },
              ]},
              { cat: 'Compartir', emoji: '🥟', color: '#d4943a', items: [
                { nombre: 'Otosan Kani', precio: '$33.600', emoji: '🦀' },
                { nombre: 'Dumplings', precio: '$27.400', emoji: '🥟' },
                { nombre: 'Burosu', precio: '$39.900', emoji: '🍜' },
              ]},
              { cat: 'Robata', emoji: '🔥', color: '#e05050', items: [
                { nombre: 'Pulpo Ton', precio: '$56.800', emoji: '🐙' },
                { nombre: 'Yakitori', precio: '$42.600', emoji: '🍢' },
                { nombre: 'Arroz Ginza', precio: '$79.900', emoji: '🥩' },
              ]},
              { cat: 'Postre', emoji: '🍮', color: '#f0b45a', items: [
                { nombre: 'Cheesecake', precio: '$32.500', emoji: '🍰' },
                { nombre: 'Koujun', precio: '$34.800', emoji: '🍮' },
                { nombre: 'Kyoto', precio: '$84.400', emoji: '🍱' },
              ]},
              { cat: 'Café/Té', emoji: '☕', color: '#cd853f', items: [
                { nombre: 'Espresso', precio: '$7.600', emoji: '☕' },
                { nombre: 'Americano', precio: '$9.200', emoji: '☕' },
                { nombre: 'Té Verde', precio: '$15.600', emoji: '🍵' },
              ]},
              { cat: 'Vino', emoji: '🍷', color: '#e91e8c', items: [
                { nombre: 'Copa Malbec', precio: '$28.000', emoji: '🍷' },
                { nombre: 'Copa Rosé', precio: '$26.000', emoji: '🥂' },
                { nombre: 'Copa Blanco', precio: '$24.000', emoji: '🍾' },
              ]},
              { cat: 'Licor', emoji: '🥂', color: '#ffd700', items: [
                { nombre: 'Sake Durazno', precio: '$44.600', emoji: '🍶' },
                { nombre: 'Heineken', precio: '$15.400', emoji: '🍺' },
                { nombre: 'Old Fashion', precio: '$48.400', emoji: '🥃' },
              ]},
            ].map(({ cat, emoji, color, items }) => (
              <div key={cat} className="flex flex-col shrink-0 border-r border-[#1a1a1a] last:border-r-0">
                <div className="px-2 py-0.5 bg-[#0a0a0a] border-b border-[#1a1a1a] flex items-center gap-1">
                  <span className="text-[11px]">{emoji}</span>
                  <span className="text-[8px] font-bold uppercase tracking-wider whitespace-nowrap" style={{ color }}>{cat}</span>
                </div>
                <div className="flex gap-1 px-1 py-1">
                  {items.map(item => (
                    <button key={item.nombre}
                      onClick={() => agregarAOrden(item)}
                      className="flex flex-col items-center gap-0.5 px-1.5 py-1 rounded-lg bg-[#141414] border border-[#1a1a1a] hover:border-[#3dba6f]/50 hover:bg-[#3dba6f]/5 active:bg-[#3dba6f] active:border-[#3dba6f] transition-all min-w-[46px]">
                      <span className="text-[15px]">{item.emoji}</span>
                      <span className="text-[7px] text-[#606060] whitespace-nowrap">{item.nombre.split(' ')[0]}</span>
                      <span className="text-[7px] font-bold" style={{ color }}>{item.precio.replace('$','').replace(/\.000/,'.0k').replace(/\.900/,'.9k').replace(/\.800/,'.8k').replace(/\.600/,'.6k').replace(/\.400/,'.4k').replace(/\.200/,'.2k').replace(/\.600/,'.6k')}</span>
                    </button>
                  ))}
                </div>
              </div>
          ...
          {/* IA Recs row */}
          <div className="flex items-center gap-0 px-3 py-1.5 overflow-x-auto" style={{ scrollbarWidth: 'none', minHeight: 52 }}>
            <div className="text-[9px] font-bold text-[#606060] tracking-[0.6px] uppercase whitespace-nowrap mr-2.5 shrink-0 flex flex-col gap-0.5 items-center">
              <span className="text-[12px]">✦</span>IA
            </div>
            {recs.map((r, i) => (
              <div key={i} onClick={() => addToOrder({ nombre: r.name, precio: r.precio, emoji: r.emoji })}
                className={`flex items-center gap-2 bg-[#141414] border rounded-lg p-1.5 px-2 min-w-[140px] max-w-[170px] shrink-0 cursor-pointer transition-all mr-1.5 hover:bg-[#3dba6f]/5 hover:border-[#3dba6f]/45 active:bg-[#3dba6f]/20 ${r.top ? 'border-[#d4943a]/30' : 'border-[#1a1a1a]'}`}>
                <span className="text-[18px] shrink-0">{r.emoji}</span>
                <div className="flex-1 min-w-0">
                  <div className="text-[10px] font-bold text-[#f0f0f0] overflow-hidden text-ellipsis whitespace-nowrap">{r.name}</div>
                  <div className="text-[9px] text-[#d4943a] font-bold">{r.precio}</div>
                </div>
              </div>
            ))}
          </div>
