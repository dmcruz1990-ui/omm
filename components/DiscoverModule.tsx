...
const DiscoverModule: React.FC = () => {
  ...
  // Si showConcierge es true, renderizamos solo el chat
  if (showConcierge) {
    return (
  ...
  // Si showConcierge es false, renderizamos el contenido normal
  return (
    <div className="space-y-16 max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-1000 pb-20">
      ...
      <section className="py-20 text-center">
        <button 
          onClick={() => setShowConcierge(true)}
          className="relative z-10 bg-white text-black hover:bg-blue-600 hover:text-white px-16 py-6 rounded-[2.5rem] font-black italic text-sm uppercase tracking-widest shadow-2xl transition-all active:scale-95 flex items-center gap-4 mx-auto group"
        >
          <Play size={20} fill="currentColor" className="group-hover:translate-x-1 transition-transform" />
          Hacer Reserva Inteligente
        </button>
      </section>
    </div>
  );
